AFRAME.registerComponent("gaussian-splatting", {
  ...
  init: function () {
    // Store the source path to use for reloading
    this.sourcePath = this.data.src;
    console.log(
      "Initializing gaussian splatting with source:",
      this.sourcePath
    );
    ...
    // Wait for the scene to be fully loaded before initializing
    if (this.el.sceneEl.hasLoaded) {
      this.setupGaussian();
    } else {
      this.el.sceneEl.addEventListener("loaded", this.setupGaussian.bind(this));
    }

    // Listen for scene changes to reinitialize if needed
  ...
  // New method to setup the gaussian properly
  setupGaussian: function () {
    console.log("Setting up gaussian splat with camera");

    try {
      // Get necessary components for rendering
      if (!this.el.sceneEl.camera || !this.el.sceneEl.camera.el) {
        console.warn("Camera not ready yet, will retry in 100ms");
        setTimeout(() => this.setupGaussian(), 100);
        return;
      }

      const camera = this.el.sceneEl.camera.el.components.camera.camera;
      const object3D = this.el.object3D;
      const renderer = this.el.sceneEl.renderer;

      // Load the data
      this.loadData(camera, object3D, renderer, this.sourcePath);

      if (this.data.cutoutEntity) {
        this.cutout = this.data.cutoutEntity.object3D;
      }
    } catch (error) {
      console.error("Error setting up gaussian splat:", error);
      // Retry after a delay
      setTimeout(() => this.setupGaussian(), 500);
    }
  },

  // Handler for scene changes
  handleSceneChange: function () {
    console.log("Scene changed, reloading gaussian splat");
  ...
  // Update method to handle changes to the component attributes
  update: function (oldData) {
    // If src changes, reload the data
    if (oldData.src !== this.data.src) {
      this.sourcePath = this.data.src;
      console.log("Source changed, reloading:", this.sourcePath);
      if (this.el.sceneEl.hasLoaded) {
        this.setupGaussian();
      }
    }
